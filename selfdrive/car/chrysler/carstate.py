...
from openpilot.selfdrive.car.chrysler.values import ChryslerFlags, DBC, STEER_THRESHOLD, RAM_CARS, CUSW_CARS
...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, frogpilot_toggles):
    ...
    if self.CP.carFingerprint in CUSW_CARS:
      return self.update_cusw(self, cp, cp_cam, frogpilot_toggles)
  ...
  def update_cusw(self, cp, cp_cam, frogpilot_toggles):
    ret = car.CarState.new_message()
    fp_ret = custom.FrogPilotCarState.new_message()
    ...
    # FrogPilot CarState functions
    fp_ret.brakeLights = bool(cp.vl["BRAKE_2"]["BRAKE_HUMAN"])
